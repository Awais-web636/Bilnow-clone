    ...
    <div className="max-w-7xl mx-auto px-12 py-16 bg-[#EDF3F6]">
      ...
      <Slider {...settings} className="revenue-slider -mx-4">
        {slides.map((slide, index) => {
          ...
          return (
            <div key={index} className="px-4" onMouseEnter={() => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)}>
              <div
                className={`relative h-full p-8 rounded-lg transition-all duration-300 overflow-hidden cursor-pointer ${
                  index === 1
